      ...
      <div className="flex-1 px-8 py-7 grid grid-cols-[1fr_360px] gap-7">
        ...
        <div className="flex flex-col gap-5">
          ...
          <button onClick={handleCreateTicket} disabled={loading}
            className="w-full h-[60px] rounded-2xl text-white text-[16px] font-black transition-all hover:opacity-90 active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50"
    ...
    <div className="h-full flex flex-col bg-[#F0F4F8]">
      ...
      <div className="bg-white border-b border-[#E2E8F0] flex-shrink-0 px-6 py-0 flex items-center" style={{ height: 56, boxShadow: "0 1px 0 #F1F5F9" }}>
        ...
        <div className="flex items-center gap-3 mr-4">
          <div className="w-7 h-7 rounded-full bg-[#DBEAFE] flex items-center justify-center text-[#1D4ED8] text-[11px] font-black">
            {user?.name?.[0] || "C"}
          </div>
          <div className="text-right">
            <p className="text-[#0F172A] text-[12px] font-semibold leading-none">{user?.name || "Rajan K"}</p>
            <p className="text-[#94A3B8] text-[10px]">On Duty</p>
          </div>
        </div>

        <button onClick={onLogout} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[#EF4444] hover:bg-[#FEF2F2] text-[12px] font-bold transition-colors">
